...
const getAllPatients = async (req, res) => {
  ...
  for (let patient of patients) {
    ...
    patient.lastVisitServiceDone = lastCompletedAppointment
      ? lastCompletedAppointment.Service.ServiceName
      : null;
...
const createNewPatient = async (req, res) => {
  ...
  const existingPatientSSN = await prisma.patient.findUnique({
    where: { PatientSSN },
  });
  ...
  if (existingPatientSSN) {
    return res
      .status(400)
      .json({ message: "A patient with this SSN already exists" });
  }
...
const updatePatient = async (req, res) => {
  ...
  // Check if a patient with the given PatientSSN or email already exists
  const existingPatientSSN = await prisma.patient.findUnique({
    where: { PatientSSN: req.body.PatientSSN },
  });
  const existingPatientEmail = await prisma.patient.findUnique({
...
const deletePatient = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({
        message:
          "An error occurred while deleting the patient and related records",
      });
  }
};
...
const createNewDoctor = async (req, res) => {
  ...
  if (existingDentist) {
    return res
      .status(400)
      .json({ message: "A dentist with this SSN already exists" });
  }
  ...
  const dentist = await prisma.dentist.create({
    data: {
      ...
      workingDays: {
        // Insert working days into the DentistWorkingDays table
        create: workingDays.map((day) => ({ day })),
      },
      workingHours: {
        // Insert working hours into the DentistWorkingHours table
  ...
  try {
    ...
    for (let workingDay of relatedWorkingDays) {
      await prisma.dentistWorkingDays.delete({
        where: {
          DentistSsn_day: { DentistSsn: dentistSsn, day: workingDay.day },
        },
      });
    }
    ...
    res
      ...
      .json({
        error:
          "An error occurred while deleting the doctor and related records.",
      });
  }
};
...
const createNewAppointmentAndInvoice = async (req, res) => {
  const { patientId, dentistSsn, date, time, serviceId, employeeSsn } =
    req.body;
  ...
  try {
    ...
    res
      .status(201)
      .json({
        message: "Appointment and associated invoice created successfully.",
      });
  } catch (error) {
    console.error(
      "An error occurred while creating appointment and invoice:",
      error
    );
    res
      .status(500)
      .json({
        error: "An error occurred while creating appointment and invoice.",
      });
  }
};
...
const getAllInvoices = async (req, res) => {
  ...
  const invoiceDetails = invoices.map((invoice) => {
    ...
    const costAfterInsuranceCoverage =
      invoice.TotalCost * (1 - insuranceCoveragePercentage);
...
const toggleInvoiceStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while updating invoice status." });
  }
};
...
const getAllItems = async (req, res) => {
  ...
  const formattedItems = items.map((item) => {
    ...
    const mostRecentCost = item.PurchaseDetails.sort(
...
const createItem = async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Item and associated purchase details created successfully.",
    });
  } catch (error) {
    console.error(
      "An error occurred while creating item and purchase details:",
      error
    );
    res
      .status(500)
      .json({
        error: "An error occurred while creating item and purchase details.",
      });
  }
};
...
const deleteItem = async (req, res) => {
  ...
  if (isNaN(itemId)) {
    return res
      .status(400)
      .json({ error: "Invalid itemId parameter. It must be an integer." });
  }
  ...
  try {
    ...
    res.json({
      message: "Item and associated purchase details deleted successfully.",
    });
  } catch (error) {
    console.error(
      "An error occurred while deleting item and purchase details:",
      error
    );
    res
      .status(500)
      .json({
        error: "An error occurred while deleting item and purchase details.",
      });
  }
};
...
const getItemPurchaseDetails = async (req, res) => {
  ...
  try {
    ...
    if (purchaseDetails.length === 0) {
      return res
        .status(400)
        .json({ message: "No purchase details found for this item" });
    }
    ...
    console.error(
      "An error occurred while retrieving item purchase details:",
      error
    );
    res
      .status(500)
      .json({
        error: "An error occurred while retrieving item purchase details.",
      });
  }
};
...
const createPurchase = async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Purchase created and item quantity updated successfully.",
    });
  } catch (error) {
    console.error(
      "An error occurred while creating purchase and updating item quantity:",
      error
    );
    res
      .status(500)
...
const decrementItemQuantity = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while decrementing item quantity." });
  }
};
...
const updateServiceCosts = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while updating service costs." });
  }
};
